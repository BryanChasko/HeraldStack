...
validate_directory_names() {
  ...
  # Find all directories, exclude hidden and special directories
  while IFS= read -r dir; do
    dirs+=("$dir")
  done < <(find "$path" -type d | grep -v "^\./\." | grep -v "^\./node_modules" | grep -v "^\./target" | sort)
  ...
  for dir in "${dirs[@]}"; do
    ...
    if [[ "$dirname" =~ [A-Z] ]] && [[ "$dir" != *"/ai-entities"* ]]; then
